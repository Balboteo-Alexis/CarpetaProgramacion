...
//  2 Declarar 3 variables de referencia
		CuentaBancaria balboteo_1,balboteo_2,balboteo_3;
...
//4 indica que cuenta tiene mayor saldo
		...
		if(balboteo_1.getSaldo()<balboteo_2.getSaldo()) {
			...
			if(balboteo_2.getSaldo()<balboteo_3.getSaldo()) {
				System.out.printf("La cuenta con mayor saldo es balboteo_%d", balboteo_3.getId()+1);
				
			}else {
				System.out.printf("La cuenta con mayor saldo es balboteo_%d", balboteo_2.getId()+1);
			}
			...
			System.out.printf("La cuenta con mayor saldo es balboteo_%d", balboteo_1.getId()+1);
		}
		...
		while(cont!= 10 || balboteo_1.isDescubierta()) {
			...
			case 0,2,4,6,8: {
				try {
					...
					balboteo_1.ingresar(saldoAleatorio());
			...
			default:
				try {
					...
					balboteo_1.extraer(saldoAleatorio());
			...
			System.out.printf("Operación: %d ,La cuenta se ha quedado con un total de: %.0f euros.%n", cont+1, balboteo_1.getSaldo());
			cont ++;
